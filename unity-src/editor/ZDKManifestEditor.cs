...
{
	public class ZDKManifest
	{
		private static string unityActivityName = "com.unity3d.player.UnityPlayerActivity";
		...
		public static bool CheckAndFixManifest()
		{
			...
			XmlElement unityActivityElement = FindElementWithAndroidName("activity", "name", ns, unityActivityName, dict);
			if (unityActivityElement == null)
			{
				Debug.LogError(string.Format("{0} activity is missing from your android manifest. Add \"<meta-data android:name=\"unityplayer.ForwardNativeEventsToDalvik\" android:value=\"true\" />\" to your activity tag so that Zendesk can display.", unityActivityName));
				return false;
			}
			...
			{
				Debug.Log("Zendesk SDK requires the minSdkVersion to be atleast 9 (Android 2.3 and up), please update the same in the manifest file for Zendesk sdk to work properly");
				return false;
			}
